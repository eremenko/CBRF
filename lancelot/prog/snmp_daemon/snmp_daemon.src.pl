...
use lib "@INSTALL_LIBDIR@/perl5/site_perl/$Config{PERL_REVISION}.$Config{PERL_VERSION}.$Config{PERL_SUBVERSION}/mach" ;
...
sub new ($$;$) {
	...
	$self->{'_kos_host'} = exists ($self->{'np_kos_host'}) ? $self->{'np_kos_host'} : 'localhost' ;
	delete $self->{'np_kos_host'} ;

	$self->{'_kos_port'} = exists ($self->{'np_kos_port'}) ? $self->{'np_kos_port'} : 3246 ;
	delete $self->{'np_kos_port'} ;
...
sub Send_Trap_To_KOS {
	...
	my $socket = IO::Socket::INET->new ('Proto'=>'tcp',
		'PeerAddr'=>$self->{'_kos_host'},
       		'PeerPort'=>$self->{'_kos_port'}) or 
	do {
       		$self->Error("Can't connect to `%s:%d': $!\n",$self->{'_kos_host'},$self->{'_kos_port'}) ;
	} ;
	...
	my ($salt) ;
	foreach my $msg (@{$ref}) {
		$self->Log('info',"Send_KOS[$$]: `".$msg."'") ;
		if (!$is_first) {
 			my $cipher = Crypt::CBC->new (-key=>$secret_password,-cipher=>'Blowfish') ;
                	my $sha = Digest::SHA->new(256) ;
                	$sha->add($salt) ;

        		my $str = sprintf ("SNMP %s %s %s \"%s\"\n",
				"#".do{my @a=split(m{},(split(m{::},(caller(0))[3]))[0]);join("",@a[0,3]);}."#",
				$sha->hexdigest(),
				$cipher->encrypt_hex($salt),
				$msg
			) ;
        		my $rc = printf $socket $str ;
        		if (!$rc) {
                                $self->Error("`%s:%d' connection error %s\n",$self->{'_kos_host'},$self->{'_kos_port'},$socket->error() || $!) ;
               		 	$socket->close() ;
                		return ;
        		}
		}

# Читаем отзыв сервера
...
# Делим его на части
		my ($reply,$dummy) ;
        	($reply,$salt,$dummy) = split (m{\s+},$line,3) ;
# Если нет salt - громко умираем, бо неясно, что в этом случае делать
        	if (!defined ($salt)) {
                        $self->Error("`%s:%d' bad protocol\n",$self->{'_kos_host'},$self->{'_kos_port'}) ;
			next ;
        	}
# Если это была ошибка, сообщаем об этом
        	if ($reply ne "OK") {
                        $self->Error("`%s:%d' server error: %s %s\n",$self->{'_kos_host'},$self->{'_kos_port'},$reply,$dummy) ;
        	}
        	else {
			if ($is_first) {
				$is_first = 0 ;
...
# парсим остатки ответа, которого нету
        	}
	}
	$socket->close() ;
	undef $socket ;
